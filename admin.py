...
# bot.py-ൽ നിന്ന് ആവശ്യമായ കാര്യങ്ങൾ ഇമ്പോർട്ട് ചെയ്യുന്നു
from bot import (
...
async def admin_panel(client, message):
    ...
    buttons = [
        ...
        [InlineKeyboardButton("📢 Broadcast", callback_data="menu_broadcast"),
         InlineKeyboardButton("🔐 FSub", callback_data="menu_fsub")],
        ...
        [InlineKeyboardButton("👑 Admins", callback_data="menu_admins"),
         InlineKeyboardButton("🛠 Maintenance", callback_data="menu_maint")],
...
• `/setstartpic [link/reply]` - Change start photo
...
• `/setlang [lang]` - Filter languages"""
        buttons = [
            [InlineKeyboardButton("🔥 Trending", callback_data="admin_trend"),
             InlineKeyboardButton("❌ Delete", callback_data="admin_delinfo")],
            [InlineKeyboardButton("🔙 Back", callback_data="admin_home")]
        ]
        await callback_query.message.edit_text(text, reply_markup=InlineKeyboardMarkup(buttons))
...
• `/setwebsite off` - Disable online watch"""
        buttons = [[InlineKeyboardButton("🔙 Back", callback_data="admin_home")]]
        await callback_query.message.edit_text(text, reply_markup=InlineKeyboardMarkup(buttons))
...
async def admin_actions(client, callback_query):
    ...
    if action == "stats":
        ...
        s = await searches_col.count_documents({}) if searches_col is not None else 0
